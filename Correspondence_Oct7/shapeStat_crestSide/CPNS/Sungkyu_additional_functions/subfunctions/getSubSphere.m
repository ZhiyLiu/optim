...
[d n]= size(data);
Gnow = 1e+10;
%disp('Message from getSubSphere.m; here it 222222222222222222222222');
while err > TOL
    %disp('begain--------norm(c0)------------------');
    c0 = c0/norm(c0); % normalize the new candidate
%     disp(norm(c0));
...
%     disp(rot*data);
    TpData = LogNPd(rot*data); % Tangent projection by Log map 
%     disp('TpDataTpDataTpDataTpDataTpData');
%     disp(TpData);
...
%     TpData = real(TpData);
    ...
    [newCenterTp r]= LMFsphereFit(TpData,zeros(d-1,1),geodesic);  % Fit a circle at Tp
%     disp('Message from getSubSphere.m; here it qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq');
    if r > pi
...
%     disp('Message from getSubSphere.m; here it iiiiiiiiiiiiiiiiiiiiiiiiiiiiii');
    newCenter = ExpNPd(newCenterTp); % Bring back to the sphere by Exp map
    center = rot\newCenter;  % (inv(rot)*newCenter) rotate back the newCenter
%     disp('centercentercentercentercentercenter: ');
%     disp(center);
    Gnext = objfn(center,r,data);
%     disp('GnextGnextGnextGnextGnext: ');
%     disp(Gnext);
    err = abs(Gnow - Gnext) ;
%     disp('errerrerrerrerrerrerrerrerrerr: ');
%     disp(err);
    Gnow = Gnext;
    c0 = center;
    cnt = cnt+1;
%         disp('cntcntcntcntcnt: ');
%         disp(cnt);
    if cnt > 30;
       %disp(['Message from getSubSphere.m: (svd) iteration reached 30th with error ' num2str(err)]);
       break;
    end
%     disp('77777777777777777777777777777777: ');
end
% disp('vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv: ');
i1save.Gnow = Gnow;    
i1save.center= center;    
i1save.r = r;
% disp('Message from getSubSphere.m; here it 3333333333333333333333333');
...
[d n]= size(data);
Gnow = 1e+10;
% disp('Message from getSubSphere.m; here it 44444444444444444444444444');
while err > TOL
    c0 = c0/norm(c0); % normalize the new candidate
    rot = rotMat(c0);  % rotation matrix : c0 -> North Pole
%     disp('rotMat while err > TOL is:');
%     disp(rot);
%         disp('rot*data  while err > TOL is: ');
%     disp(rot*data);
    TpData = LogNPd(rot*data); % Tangent projection by Log map 
%     disp('TpData while err > TOL is:');
%     disp(TpData);
    
    %Liyun Dec 3, 2013. ---------
%     TpData = real(TpData);
    
%     disp('TpData while throw away the imaginary part:');
%     disp(TpData);
    %----------------------------
    
    
    [newCenterTp r]= LMFsphereFit(TpData,zeros(d-1,1),geodesic);  % Fit a circle at Tp
%     disp('Message from getSubSphere.m; here it 5555555555555555555555555');
    if r > pi
...
% disp('Message from getSubSphere.m; here it 66666666666666666666666666666');
