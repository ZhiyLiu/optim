...
int main( int argc, char *argv[] )
{
  if (argc < 3)
  {
    ...
    std::cout << "-comp: Compute compactness" << std::endl;
    std::cout << std::endl ;
    return -1;
  }
  ...
  if ( argc > 3 )
  {
      ...
      if (( !strcmp ( "-numSpec", argv[i] ) ) )
    {
      ...
      if (( !strcmp ( "-comp", argv[i] ) ) )
    {
      computeCompactness = true;
    }
    }
  }
  ...
  std::vector < double > generalization, specificity, compactness ;
  std::vector < double > generalizationError, specificityError, compactnessError ;
  generalization.resize ( numSamples - 1 ) ;
  specificity.resize ( numSamples ) ;
  compactness.resize(numSamples);
  generalizationError.resize ( numSamples - 1 ) ;
  specificityError.resize ( numSamples ) ;
  compactnessError.resize(numSamples);
    ...
    for ( unsigned int i = 0 ; i < numSamples ; i++ )
  {
    ...
    if(computeCompactness) {
        // compute the model
        std::vector < double > eigValues = evaluator->GetModelVariance();

        for ( unsigned int i = 0 ; i < numSamples ; i++ ) {
            std::cout<<"Starting compactness computation with "<< i<< " shape parameters... ";
            compactness[i] = evaluator->GetCompactness(i, compactnessError[i], eigValues);
            std::cout << "Finished." << std::endl ;
        }
    }
  ...
  if ( computeCompactness )
  {
    outputFile << "Compactness: " << std::endl ;
    for ( unsigned int i = 0 ; i < numSamples ; i++ )
    {
      outputFile << i << " " << compactness[i] << " " << compactnessError[i] << std::endl ;
    }
  }
  outputFile.close () ;
