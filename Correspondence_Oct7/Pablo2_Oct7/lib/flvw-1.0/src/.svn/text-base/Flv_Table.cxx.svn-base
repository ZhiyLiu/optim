...
// Added use of FL_Tab, 21 July 2001, A. G. Gash
...
int Flv_Table::handle(int event)
{
	...
	// AGG: Added this block so leaving the Flvt_Edit object without
	// having typed RETURN after other text should update the cells.
	if (event == FL_LEAVE && vediting) {
			...
			switch( vmove_on_enter)
			{
				case FLV_MOVE_ON_ENTER_ROW_COL:
					if (!move_row(1))
					{
						...
						if (!select_locked())
						{
							r = 0;		// Added by AGG to correct use of initialized variable
							get_style(s,r,col());
							if (!s.locked())
								move_row(1);
						}
