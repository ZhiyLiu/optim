...
public:
	...
	// dibyendu
	// function to write out all distances at the end of all spokes
	bool writeDistancesAtSpokeEnds(const char * filename) ;
...
//    void resetSubfigureOptimizer();
	...
	// SRep stage optimization functions
    bool initializeSRepOptimizer(int appID = 0);
    M3DSRepOptimizer * getSRepOptimizer() const { return sRepOptimizer; }
    void resetSRepOptimizer();
    bool doSRepIterations(int figureId, bool preview = false, bool verbosity = false);
    double getLastSRepObjectiveFunctionValue(bool preview = false);
    double getLastSRepGeometricMatchValue();
	...
	// dibyendu - cpns
    bool cpnsDeform(std::vector<double> & vals);
	bool haveCPNS();
	void resetCPNSStage();
	...
	//dibyendu
	bool binaryPabloSRepStage(int maxNumIterations);
...
protected:
	...
	//dibyendu
	M3DSRepOptimizer * sRepOptimizer;
	...
	*/
	...
	// dibyendu - cpns
	// Pointers to the CPNS statistic objects in object 
