...
M3DMainFigResidueCPNSProblem::M3DMainFigResidueCPNSProblem( M3DCPNSStats *cpnsPtr, int _order)
{
	cpns = cpnsPtr;
	order = _order;
}

int M3DMainFigResidueCPNSProblem::parameterCount() const
{
	return cpns->getNEigenModes();
}

double M3DMainFigResidueCPNSProblem::evaluate(const Vector &x)
{
	double geomPenalty;
	double totalPenalty;
	...
	totalPenalty = match->computeFigureMatchAndPenalties(targetObject, figureId, NULL, vals, geomPenalty);
...
M3DObject * M3DMainFigResidueCPNSProblem::createTargetObject(const Vector &x, int &figureId, bool predict)
{
	...
	double * x2 = new double[vals.size()];

	for (int i = 0; i < vals.size(); i++)
		x2[i] = vals[i];
	
	// FIX: INCLUDE WEIGHTS
	targetObject = cpns->eigenmodeDeformMean(x2, refObject);
...
M3DMainFigResidueCPNSProblemBoundsFunction::M3DMainFigResidueCPNSProblemBoundsFunction()
{
