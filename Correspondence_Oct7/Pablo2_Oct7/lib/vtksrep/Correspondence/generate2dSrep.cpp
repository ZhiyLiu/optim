...
int main(int argc, char** argv)
{
    if(argc < 4)
    {
        std::cout << "Usage: gen2dSrep <modelFile> <atomRows> <atomColumns>" << std::endl;
        return -1;
    }
    char *modelName = argv[1];
    int nRows = atoi(argv[2]); // rows and cols of input srep model
    int nCols = atoi(argv[3]);
    M3DObject* object = readModel(modelName);
    M3DFigure* figure = object->getFigurePtr(0);
    M3DQuadFigure* quadFigure = dynamic_cast<M3DQuadFigure*>(figure);

    // 1. compute the mean of z as cutting position
    ...
    {
        M3DPrimitive* p = figure->getPrimitivePtr(i);
        p->deselect();
        // the furthest must be crest spoke
        M3DQuadEndPrimitive* endPrimitive = dynamic_cast<M3DQuadEndPrimitive*>(p);
        if(endPrimitive != NULL)
        {
            double r = endPrimitive->getREnd();
            Vector3D uend = endPrimitive->getUEnd();
            Vector3D pos = endPrimitive->getX();

            double z = pos.getZ()+r*uend.getZ();
            if(minZ > z)
            {
                minZ = z;
                idxMin = i;
    ...
    M3DPrimitive* p = figure->getPrimitivePtr(idxMin);
    p->select();
    
    p = figure->getPrimitivePtr(idxMax);
    p->select();

    meanZ = 0.5*(minZ+maxZ);
    // 2. interpolate at cutting position
    // interpolate based on the middle most in column direction (long axis)
    // in u, v space, u=row v = col
    int v = (floor)(nCols/2);
    for(int u = 0; u < nRows; ++u)
    {
        M3DPrimitive* p_base = quadFigure->getPrimitivePtr(u,v);
        M3DQuadPrimitive* pbase = dynamic_cast<M3DQuadPrimitive*>(p_base);
        pbase->select();
        Vector3D pos = pbase->getX();
        double delta = pos.getZ()-meanZ;
        // M3DSpoke* newUp = interpolateSpoke(figure, u, meanZ, 0);
        // M3DSpoke* newDown = interpolateSpoke(figure, u, meanZ, 1);
        // double z1 = newUp->getX().getZ();
        // double z2 = newDown->getX().getZ();
        pos.setZ(meanZ);
        pbase->setX(pos);
        Vector3D u0 = pbase->getU0();
        u0.setZ(0);
        u0.normalize();
        pbase->setU0(u0);

        Vector3D u1 = pbase->getU1();
        u1.setZ(0);
        u1.normalize();
        pbase->setU1(u1);
    }
    // regularize end primitive
    M3DPrimitive* p_end = quadFigure->getPrimitivePtr(0, v);
    M3DQuadEndPrimitive* pend = dynamic_cast<M3DQuadEndPrimitive*>(p_end);
    if(pend!= NULL)
    {
        Vector3D uend = pend->getUEnd();
        uend.setZ(0);
        uend.normalize();
        pend->setUEnd(uend);
    }
    p_end = quadFigure->getPrimitivePtr(nRows-1, v);
    pend = dynamic_cast<M3DQuadEndPrimitive*>(p_end);
    if(pend!= NULL)
    {
        Vector3D uend = pend->getUEnd();
        uend.setZ(0);
    ...
    M3DObjectFile objectFile;
    objectFile.write("2dsrep.m3d", *object);
