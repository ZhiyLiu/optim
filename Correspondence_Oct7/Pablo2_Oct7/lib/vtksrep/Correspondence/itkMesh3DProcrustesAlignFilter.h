...
public:
  ...
  */
  ...
  //virtual void SetInput( unsigned int idx, InputMeshPointer mesh );
/** Sets one input mesh (starting with idx=0). */
    void SetInput( unsigned int idx, InputMeshPointer mesh );
  ...
  void PrintTransform( TransformType* transform )
  {
    ...
    output.open("/NIRAL/work/ltu/WorkSpace/data_pablo/Pablo2/lib/vtksrep/Correspondence/Transform.info", std::ios::out);
  ...
  */
  ...
  virtual DataObjectPointer MakeOutput(itk::ProcessObject::DataObjectPointerArraySizeType idx);
...
protected:
  ...
  // Uses the current input to calculate a initial mean.
  void InitialMean();
...
private:
  ...
  * samller than m_Convergence, the alignment is terminated. */
  ...
  //MatrixType trans_source;

  //std::vector<InputMeshPointer> transformed_X; //holding the changing transformed mesh.
