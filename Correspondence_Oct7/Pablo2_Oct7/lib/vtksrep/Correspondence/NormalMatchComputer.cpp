/* The purpose of this class is to provide a base class for kind of metric computer
 *
 * Zhiyuan Liu
 * 2018.02
 */
...
void NormalMatchComputer::setTargetImage(ImageDistanceMap* target)
{
    mDistanceImage = target;
}

void NormalMatchComputer::setSrepModel(M3DObject* sreps)
{
    mSreps = sreps;
}

void NormalMatchComputer::setTargetFigureIndex(int figureId)
{
    mFigureIndex = figureId;
}

void NormalMatchComputer::setFigure(M3DQuadFigure* f)
{
    if(mFigure != NULL)
    {
        delete mFigure;
...
bool NormalMatchComputer::compute(double *output)
{
    ...
    for(int i = 0; i < primitiveCount; ++i)
    {
        ...
        double topDistance = getSpokeNormalMatch(topNeighbors) * R0;
        double botDistance = getSpokeNormalMatch(botNeighbors) * R1;
        ...
        if(endPrimitive != NULL)
        {
            ...
            double endDistance = getSpokeNormalMatch(endNeighbors);
...
double NormalMatchComputer::getSpokeNormalMatch(std::vector<M3DSpoke*>& spokes)
{
    double result = 0.0;
...
void NormalMatchComputer::getRelevantSpokes(M3DFigure* f, M3DQuadPrimitive* quadAtom, int atomId, int spokeId, std::vector<M3DSpoke*>* outSpokes)
{
...
void NormalMatchComputer::GetCrestSpokes(vtkSmartPointer<vtkSRep> srepfig, int level, vector<M3DSpoke*>& spokes,bool istube, vtkIdType vtkAtomId, int spokeId){
...
vtkIdType NormalMatchComputer::GetVtkSrepFig(M3DFigure* figure, vtkSmartPointer<vtkSRep>& srepfig, int atomId){
...
M3DSpoke* NormalMatchComputer::interpolateSpoke(M3DFigure *figure, double u, double v, int side)
{
