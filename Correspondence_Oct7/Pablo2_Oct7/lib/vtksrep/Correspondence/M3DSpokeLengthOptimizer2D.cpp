...
#include "SimilarityComputer2D.h"
#include "M3DObjectFile.h"

M3DSpokeLengthOptimizer2D::M3DSpokeLengthOptimizer2D()
    :mSreps(NULL){}

M3DSpokeLengthOptimizer2D::M3DSpokeLengthOptimizer2D(M3DObject* sreps)
{
    if(sreps != NULL)
    {
        mSreps = sreps;
...
double M3DSpokeLengthOptimizer2D::computeSradPenalty()
{
...
void M3DSpokeLengthOptimizer2D::updateFigure(const double *coeff, int figureId)
{
    try
    {
...
double M3DSpokeLengthOptimizer2D::getObjectiveFunctionValue(const double *coeff, double w1, double w2)
{
    // input coeff from newuoa is the coeff of length. it could be positive or negative

    // 0. Update new length to each spoke
//    M3DQuadFigure* newFigure = NULL;
    ...
    // 1. Image match
    SimilarityComputer2D similarityCompter;
...
void M3DSpokeLengthOptimizer2D::recoverFigure(int figureId)
{
    M3DFigure* figure = mSreps->getFigurePtr(mFigureIndex);
    int primitiveCount = figure->getPrimitiveCount();
    int spokeId = 0;
    for(int i = 0; i < primitiveCount; ++i)
    {
        ...
        quadAtom->setR0(mOriginalRadii[spokeId]);
        quadAtom->setR1(mOriginalRadii[spokeId + 1]);
        ...
        {
//            continue;
            endPrimitive->setREnd(mOriginalRadii[spokeId+2]);
            spokeId += 1;
        }
        spokeId += 2;
...
double M3DSpokeLengthOptimizer2D::operator() (double *coeff)
{
    double cost = 0.0;
...
void M3DSpokeLengthOptimizer2D::setImage(ImageDistanceMap* imageDistanceMap)
{
    mSignedDistanceImage = imageDistanceMap;
}

void M3DSpokeLengthOptimizer2D::setObject(M3DObject* sreps)
{
    mSreps = sreps;
}

/* Main entry of optimizer */
int M3DSpokeLengthOptimizer2D::perform(M3DObject* outputModel)
{
    ...
    for(int i = 0; i < primitiveCount; ++i)
    {
        ...
        mOriginalRadii.push_back(R0);
        mOriginalRadii.push_back(R1);
        
        if(endPrimitive != NULL)
        {
//            continue;
            double r = endPrimitive->getREnd();
            mOriginalRadii.push_back(r);
        }


    }
    int spokeCount = mOriginalRadii.size();
    ...
    min_newuoa(mOriginalRadii.size(),coeffOfLength,*this,newuoaParams[0], newuoaParams[1], newuoaParams[2]);
    ...
    for(int i = 0; i < primitiveCount; ++i)
    {
        ...
        double r0 = mOriginalRadii[spokeId] * exp(coeffOfLength[spokeId]);
        double r1 = mOriginalRadii[spokeId + 1] * exp(coeffOfLength[spokeId + 1]);
        quadAtom->setR0(r0);
        quadAtom->setR1(r1);
        ...
        {
//            continue;
            double r = mOriginalRadii[spokeId + 2] * exp(coeffOfLength[spokeId + 2]);
            endPrimitive->setREnd(r);
            spokeId += 1;
        }
        spokeId += 2;
