...
namespace itk
{
  ...
  template<class TOutputMeshType>
  std::vector < double > CorrespondenceEvaluator<TOutputMeshType>
  ::GetModelVariance()
  {
    // build model
    ...
    eigValues.resize ( this->m_NumberOfInputs ) ;
  ...
  ::GetCompactness(unsigned int numberOfShapeParameters, double &stdError, std::vector < double > eigValues) {
      ...
      double cm  = 0;
      stdError = 0;
      double temp = sqrt(2.0 / this->m_NumberOfInputs);
      for(unsigned int i = 0; i < numberOfShapeParameters; i++){
          cm += eigValues[i];

          stdError += temp * eigValues[i];
      }
