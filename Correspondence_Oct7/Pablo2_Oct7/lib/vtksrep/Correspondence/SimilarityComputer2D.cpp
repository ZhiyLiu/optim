...
SimilarityComputer2D::SimilarityComputer2D():
...
void SimilarityComputer2D::setTargetImage(ImageDistanceMap* target)
{
    mDistanceImage = target;
}

void SimilarityComputer2D::setSrepModel(M3DObject* sreps)
{
    mSreps = sreps;
}

void SimilarityComputer2D::setTargetFigureIndex(int figureId)
{
    mFigureIndex = figureId;
}

void SimilarityComputer2D::setFigure(M3DQuadFigure* f)
{
    if(mFigure != NULL)
    {
        delete mFigure;
...
double SimilarityComputer2D::getSpokeNormalMatch(std::vector<M3DSpoke*>& spokes)
{
    double result = 0.0;
...
bool SimilarityComputer2D::compute(double *similarityMeasure, double *normalMatch)
{
    // step 1: validate data
    if(mDistanceImage == NULL)
    {
        std::cout << "[Error]SimilarityComputer2D requires both target image and reference model" << std::endl;
        return false;
    }
    double match = 0.0;
    ...
    for(int i = 0; i < primitiveCount; ++i)
    {
        ...
        allSpokes.push_back(top);
        allSpokes.push_back(bottom);
...
//        totalDistance = getSSD(topArray) + getSSD(botArray);
    ...
    totalDistance = getSSD(allSpokes);
    *similarityMeasure = totalDistance;
    *normalMatch = totalNormalMatch;
    return true;
}
double SimilarityComputer2D::getSSD(std::vector<M3DSpoke>& spokes)
{
    double result = 0.0;
    for(int i = 0; i < spokes.size(); ++i)
    {
...
void SimilarityComputer2D::getRelevantSpokes(M3DFigure* f, M3DQuadPrimitive* quadAtom, int atomId, int spokeId, std::vector<M3DSpoke*>* outSpokes)
{
...
void SimilarityComputer2D::GetCrestSpokes(vtkSmartPointer<vtkSRep> srepfig, int level, vector<M3DSpoke*>& spokes,bool istube, vtkIdType vtkAtomId, int spokeId){
...
vtkIdType SimilarityComputer2D::GetVtkSrepFig(M3DFigure* figure, vtkSmartPointer<vtkSRep>& srepfig, int atomId){
    ...
    if(quadfig){
        ...
        for(int u = 0; u < quadfig->getRowCount(); u++){
            ...
            for(int v = 0; v < quadfig->getColumnCount(); v++){
                ...
                if(u == 0 || u == quadfig->getRowCount() - 1){
...
M3DSpoke* SimilarityComputer2D::interpolateSpoke(M3DFigure *figure, double u, double v, int side)
{
