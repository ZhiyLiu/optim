...
public:
   ...
   Matrix(int rows,int columns,double *_data=NULL,bool columnMajorOrder=true) {
      initStorage(rows,columns);
      initData(_data,columnMajorOrder);
   }

   // Creates a matrix and reads in values in column major order
   // Make sure that constants are specified correctly, e.g.
   //    MatrixTmp<double> (1,2,3,3) 
   // will cause problems but MatrixTmp<double> (1,2,3.0,3.0) will not.
