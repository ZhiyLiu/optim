...
#define MAX_NUM_SREP_MATCH_RESULTS	12
...
//        const char * maskFilename, bool useWindowing = true);
	...
	// Dibyendu
	// computing binary image match for a spoke and its neighborhood (interpolated ones only)

	double computeSpokeBinaryImageMatch(M3DObject * targetObject, int figureId, int atomId, int spokeId, int subdivLevel);
    
	//Xiaojie

	// ---------------------------------

	double computeFigureBinaryImageNormalMatch( M3DObject * targetObject, int figureId );

    double computeAtomBinaryImageNormalMatch( M3DObject * targetObject, int figureId, int atomId, int subdivLevel );

	// dibyendu - adding the function only for the spoke and its neighbors

    double computeSpokeBinaryImageNormalMatch( M3DObject * targetObject, int figureId, int atomId, int spokeId, int subdivLevel );
	...
	double computeSpokeBinaryImagePlaneOrienMatchAtEnd( M3DObject * targetObject, int figureId, int atomId, int spokeId );
	...
	double computeSpokeBinaryImageVertexMatch( M3DObject * targetObject, int figureId, int atomId, int spokeId );
	...
	double computeFigureRadiusMatch( M3DObject * targetObject, int figureId );

    double computeAtomRadiusMatch( M3DObject * targetObject, int figureId, int atomId );
	...
	vector <M3DSpoke*> getRelevantSpokes(M3DObject* targetObject, int level, int figureId);
	vector <M3DSpoke*> getRelevantSpokes(M3DObject* targetObject, int level, int figureId, int atomId);
	vector <M3DSpoke*> getRelevantSpokes(M3DObject* targetObject, int level, int figureId, int atomId, int spokeId);
	...
	Components computeSRepMatchAndPenaltiesAsComponents(M3DObject * targetObject,
		int figureId, int atomId, int spokeId, const Vector & x, double & geomPenalty);
	...
	double computeSRepMatchAndPenalties(M3DObject * targetObject, int figureId,
		int atomId, int spokeId, const Vector & x, double & geomPenalty)
	{
		const Components penalty =
			computeSRepMatchAndPenaltiesAsComponents(targetObject, figureId,
				atomId, spokeId, x, geomPenalty);
		return penalty.rough + penalty.smooth;
	}
	...
	// dibyendu
	// to compute the distance between this spoke and the corresponding spoke in the reference object
	double computeSpokeDistFromRef( M3DObject * targetObject, int figureId, int atomId, int spokeId, DistanceType DT = AVE_EUCLIDEAN_DIST ) ;					
	...
	const matchResult * getSRepStageResults() const { return sRepResults; }
...
private:
	...
	//dibyendu
	matchResult sRepResults[MAX_NUM_SREP_MATCH_RESULTS];
    ...
    void GetInternalSpokes(vtkSmartPointer<vtkSRep> srepfig, int level, vector<M3DSpoke*>& spokes, vtkIdType atomId = -1, int spokeId = -1);
    vtkIdType GetVtkSrepFig(M3DObject* targetObject, int figureId, vtkSmartPointer<vtkSRep>& srepfig, int atomId = -1);
