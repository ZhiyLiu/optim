...
enum bpTune_t
{
	...
	// SPEED OPTIMIZATIONS - DISTANCE MAP
	...
	BpDDMAntialiased,		// Whether antialiased distance map file is used
	...
	//xiaojie
	WeightNormalMatchAtEndMidSpoke, //weight mid spoke of ends in the process of computing image normal match.
...
//	AtomElongationFactor,	// Elongation factor (in atom stage???)
	...
	SRepSkipFigures,		// List of figures for which the SRep stage will be skipped					Not in use yet
	BpSRepResetModelBefore,	// reset reference model before SRep stage

	BpSRepSurfaceLevel,		// SRep subdivision surface level: 0 is min					// AGG: This should also apply to Gp
	SRepIterations,			// SRep maximum number of iterations

	BpSRepAverageNeighborMatch,	// AN (Average of Neighbors: model regularity); was alpha*R
	BpSRepImageMatch,		// IM (Image Match: average distance^2); was (1-alpha)*D
	SRepRadiusMatch,		// Radius match for s-reps
	BpSRepLandmarkMatch,	// LM (LandMark Match: average distance^2); was gamma*L
	BpSRepModelMatch,		// MM (Model Match: to reference model); was beta*K
	BpSRepNeighborhood,		// SRep neighborhood
	BpSRepDistanceType,		// SRep distance types
	BpSRepRSradPenalty,		// RSrad penalty to enforce legality
	BpSRepSumMatch,			// Sum over all atoms.
	BpSRepDeltaNeighborMatch,	// AN (difference in Average of Neighbors delta in reference versus candidate); was alpha*R
	BpSRepCurvinessPenalty,	// SRep stage curviness penalty
	//Xiaojie
	BpSRepImageNormalMatch,  //SRep Image Normal Match
	BpSRepImagePlaneOrienAtEnd,  //SRep Image Plane Orientation Match at End Points
	BpSRepImageVertexMatch, //Figure Image Vertex 

	// SREP STAGE (multi figure)
	BpSRepInterpenetrationMatch,	// Mask SRep stage penetration
																// AGG: The set of items below probably can be added to Gp easily
	BpSRepSkipThreshold,	// SRep skip threshold
	BpSRepSkipMultiplier,	// SRep skip multiplier
	BpSRepConjugateGradientIterations,		// Conjugate gradient iterations
	BpSRepSkippedPasses,	// Skipped passes
	BpSRepRandomOrder,		// Randomize the order of optimization of SReps

	// SRep optimizer dimensional factors and bounds
	SRepTranslationFactor,	// Translation factor
	SRepRotationFactor,		// Rotation factor
	SRepScaleFactor,		// Scale factor
	SRepThetaFactor,		// Theta factor
//	SRepElongationFactor,	// Elongation factor (in SRep stage???)
	SRepEtaFactor,			// Eta factor: 0 disables eta optimization 
	SRepPGAFactor,			// Using SRep PGA
	SRepEpsilon,			// Optimizer epsilons (step size)

	SRepAtomRestrictMovement,	// Restricts the movement of atoms (translation of the hub in the SRep stage)

	SRepTranslationBound,	// Translation bound
	SRepRotationBound,		// Rotation bound
	SRepScaleBound,			// Scale bound
	SRepThetaBound,			// Theta bound
	SRepEtaBound,			// Eta bound: 0 disables eta optimization 
	SRepPGABound,			// SRep PGA Bound
