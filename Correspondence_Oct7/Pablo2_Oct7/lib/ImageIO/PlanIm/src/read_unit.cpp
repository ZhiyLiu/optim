...
int unit_version_7(XDR_fdes *xdr_fdes, UNIT *unit);
...
{   int		ret;
	...
	case 4:
	    ...
	    break;
	case 7:
	    ret = unit_version_7(xdr_fdes, unit);
	    break;
    }
    return(ret);
}
...
unit_version_4(XDR_fdes *xdr_fdes, UNIT *unit)
{
    ...
    if (unit->version >= 5) {
	if (!(xdr_float(xdr, &unit->e_eq_diam))) {
	    XDR_ReportError("Cannot read unit");
	    return(XDR_ERROR);
	}
    }
    else unit->e_eq_diam = 0.0;
    if (!(xdr_int(xdr, &unit->x_jaw_count) &&
	  xdr_int(xdr, &unit->y_jaw_count))) {
	XDR_ReportError("Cannot read unit");
	return(XDR_ERROR);
    }
    if (unit->version >= 6) {
	if (!(xdr_int(xdr, &unit->x_jaw_type) &&
	      xdr_int(xdr, &unit->y_jaw_type))) {
	    XDR_ReportError("Cannot read jaw type");
	    return(XDR_ERROR);
	}
    }
    else {
	unit->x_jaw_type = DOUBLE_DIVERGENT;
	unit->y_jaw_type = DOUBLE_DIVERGENT;
    }
    for (loop = 0; loop < unit->x_jaw_count + unit->y_jaw_count; loop++) {
	jaw = unit->jaw + loop;
	cp1 = &(jaw->name_1[0]);
...
int
unit_version_7(XDR_fdes *xdr_fdes, UNIT *unit)
{
    int		loop;
    char	*cp, *cp1, *cp2;
    JAW		*jaw;
