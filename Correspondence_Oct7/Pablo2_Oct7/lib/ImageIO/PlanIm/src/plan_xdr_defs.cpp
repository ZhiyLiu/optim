...
- 19-jun-2002 gst: if ysize was wrongly set in a plan_im file, it's
  value can be ignored when reading. See IGNORE_YSIZE. These plan_im
  files may have been written with an interim version of PLUNC
...
***************************************************************************/
int
xdr_EXT_BEAM(XDR *xdr, EXT_BEAM *eb)
{
    char		*cp;
    unsigned int	 lp;
    unsigned int	i;
    ...
    if (xdr->x_op == XDR_ENCODE && eb->accessory_id != NO_ACCESSORY)
	eb->version = -8;
    else eb->version = -6;
    if (!(xdr_u_long(xdr, &(eb->serial_number)) &&
	  xdr_string(xdr, &cp, NAME_LENGTH) &&
	  xdr_int(xdr, &eb->version)))
	return(FALSE);
    if (eb->version <= -2) {
    ...
    if (eb->version == -6 && eb->placement == SSD_PLACEMENT) {
    ...
    if (eb->version <= -8) {
	if (!xdr_int(xdr, &eb->accessory_id))
	    return(FALSE);
    }
    else eb->accessory_id = NO_ACCESSORY;
...
{   int		i;
	...
	if (! xdr_int(xdr, &opcode)) return(FALSE);
	oplen = 4 + (((strlen(cp) + 3)>>2)<<2);    // AGG: I added the outer parentheses to fix an operator precedence error. 
	if (! xdr_int(xdr, &oplen)) return(FALSE);
    ...
    if (p->time_count > 1) {
	opcode = PLAN_IM_TIME_COUNT;
	if (! xdr_int(xdr, &opcode)) return(FALSE);
	oplen = 4;
	if (! xdr_int(xdr, &oplen)) return(FALSE);
	if (! xdr_int(xdr, &p->time_count)) return(FALSE);
    }
	...
	oplen = 4;
	if (! xdr_int(xdr, &oplen)) return(FALSE);
	if (! xdr_enum(xdr, (int *)&p->cut)) return(FALSE);
    }

    if (p->compression) {
	opcode = PLAN_IM_COMPRESSED;
	if (! xdr_int(xdr, &opcode)) return(FALSE);
	oplen = 4;
	if (! xdr_int(xdr, &oplen)) return(FALSE);
	if (! xdr_int(xdr, (int *)&p->compression)) return(FALSE);
	for (i = 0; i < p->slice_count; i++) {
	    if (! xdr_int(xdr, (int *)&p->per_scan[i].nbytes)) return(FALSE);
	}
    }
...
{   int		i;
	...
	if (min_pos > (int) p->per_scan[i].offset_ptrs)
	...
	switch (opcode) {
...
#ifdef IGNORE_YSIZE
		{
			// to read a plan_im with a bogus y_size, ignore the real value here
			float junk;
			if (! xdr_float(xdr, &junk)) return(FALSE);
		}
#else
		if (! xdr_float(xdr, &p->y_size)) return(FALSE);
#endif
		...
		if (! xdr_enum(xdr, (int *)&p->cut)) return(FALSE);
		break;
	    case PLAN_IM_COMPRESSED:
		if (oplen != 4) return(FALSE);
		if (! xdr_int(xdr, (int *)&p->compression)) return(FALSE);
		for (i = 0; i < p->slice_count; i++) {
		    if (!xdr_int(xdr, (int *)&p->per_scan[i].nbytes))
			return(FALSE);
		}
		break;
	    case PLAN_IM_END :
...
{   int		i, j;
    ...
    lp = OLD_ABSURDLY_LARGE_MAX_SCANS;
    if (lp < (unsigned) p->slice_count) lp = (unsigned) p->slice_count;
    if (! xdr_array(xdr, &cp, &lp, lp,
