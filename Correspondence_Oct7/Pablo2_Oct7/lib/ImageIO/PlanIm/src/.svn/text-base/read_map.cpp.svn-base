...
XDR_WriteMap(int fdes, THREED_MAP *map, int slice, int num_slices, char mode)
{
   ...
   if (! xdr_float(xdr_fdes->xdrs, &(map->z_start))) {
      XDR_ReportError("Cannot write z start");
      status = XDR_ERROR;
   }
   if (! xdr_float(xdr_fdes->xdrs, &(map->z_end))) {
      XDR_ReportError("Cannot write z end");
      status = XDR_ERROR;
   }
   if (! xdr_float(xdr_fdes->xdrs, &(map->z_scale))) {
      XDR_ReportError("Cannot write z scale");
      status = XDR_ERROR;
   }
   if (! xdr_float(xdr_fdes->xdrs, &(map->inv_z_scale))) {
      XDR_ReportError("Cannot write z inv scale");
      status = XDR_ERROR;
   }
