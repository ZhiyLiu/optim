...
/* ---------------------------------------------------------------------------
   ...
   NAME
 	read_accessories read_filters, read_units, read_trays
...
int
read_accessories(int *count, ACCESSORY **a)
{
    int		fdes,
		loop;
    int		lcount;
    ...
    *count = 0;
    ...
    sprintf(file_name, "%s/accessory", get_phys_dat_dir());
    fdes = open(file_name, O_RDONLY, 0);
    
    if (fdes < 0) {
	printf("file: %s\n", file_name);
	perror("read_accessories: open error");
	return -1;
    }

    if (read_int(fdes, &lcount)) {
	printf("file: %s\n", file_name);
	perror("read_accessories: read error");
	return -1;
    }
    ...
    *a = (ACCESSORY *)malloc(*count * sizeof(ACCESSORY));
    if (*a == NULL) {
	printf("file: %s\n", file_name);
	fprintf(stderr, "accessory malloc failed");
	exit(0);
    }

    for (loop = 0 ; loop < *count; loop++) {
	if (read_accessory(fdes, *a + loop)) {
	    printf("file: %s\n", file_name);
	    perror("read_accessories: read error");
	    return -1;
	}
