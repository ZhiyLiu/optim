...
M3DSpoke* M3DInterpolater::interpolateSpoke(M3DFigure *figure, double u, double v, int side)
{
    ...
    std::cout << ubase << ", " << vbase << std::endl;
    std::cout << u << ", " << v << std::endl;
    if (u == 0 && v == 0)
    {
	std::cout << "in if" << std::endl;
	M3DQuadFigure *tempFigure = dynamic_cast<M3DQuadFigure*>(figure);
	M3DPrimitive *atom = tempFigure->getPrimitivePtr(ubase, vbase);
	M3DQuadPrimitive *quadAtom = dynamic_cast<M3DQuadPrimitive*>(atom);
	
	Vector3D newB;
	if (side == 0)
	   newB = quadAtom->getX() + quadAtom->getY0();
	else
	    newB = quadAtom->getX() + quadAtom->getY1();
    ...
    M3DPrimitive *atom21 = tempFigure->getPrimitivePtr(ubase,vbase);
    M3DQuadPrimitive *quadAtom21 = dynamic_cast<M3DQuadPrimitive*>(atom21);
    M3DPrimitive *atom12 = tempFigure->getPrimitivePtr(ubase,vbase);
    M3DQuadPrimitive *quadAtom12 = dynamic_cast<M3DQuadPrimitive*>(atom12);
    M3DPrimitive *atom22 = tempFigure->getPrimitivePtr(ubase,vbase);
    ...
    Vector3D Bx11, Bx21, Bx12, Bx22;
    
    if (side == 0)
    {
	U0_11 = quadAtom11->getU0();
	Bx11 = quadAtom11->getX() + quadAtom11->getY0();
	U0_21 = quadAtom21->getU0();
	Bx21 = quadAtom21->getX() + quadAtom21->getY0();
	U0_12 = quadAtom12->getU0();
	Bx12 = quadAtom12->getX() + quadAtom12->getY0();
	U0_22 = quadAtom22->getU0();
	Bx22 = quadAtom22->getX() + quadAtom22->getY0();
    }
    else
    {
	U0_11 = quadAtom11->getU1();
	Bx11 = quadAtom11->getX() + quadAtom11->getY1();
	U0_21 = quadAtom21->getU1();
	Bx21 = quadAtom21->getX() + quadAtom21->getY1();
	U0_12 = quadAtom12->getU1();
	Bx12 = quadAtom12->getX() + quadAtom12->getY1();
	U0_22 = quadAtom22->getU1();
	Bx22 = quadAtom22->getX() + quadAtom22->getY1();
    }
    
    Vector3D du11, du21, du12, du22, dv11, dv21, dv12, dv22;
    Vector3D Bdu11, Bdu21, Bdu12, Bdu22, Bdv11, Bdv21, Bdv12, Bdv22;
    ...
    /*std::cout << "starting D block" << std::endl;
    std::cout << "Corner 11" << std::endl;
    ...
    else
    {
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdu11 = (Bx21 - tempAtom->getX() + tempAtom->getY0()) / 2;
	    }
	    else
	    {
		    dU0du11 = (U0_21 - tempAtom->getU1()) / 2;
		    Bdu11 = (Bx21 - tempAtom->getX() + tempAtom->getY1()) / 2;
	    }
    }
    ...
    else
    {
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdv11 = (Bx12 - tempAtom->getX() + tempAtom->getY0()) / 2;
	    }
	    else
	    {
		    dU0dv11 = (U0_12 - tempAtom->getU1()) / 2;
		    Bdv11 = (Bx12 - tempAtom->getX() + tempAtom->getY1()) / 2;
	    }
    }

    // Corner 21
    std::cout << "Corner 21" << std::endl;
	...
	std::cout << ubase + 1 << ", " << tempFigure->getRowCount() - 1 << std::endl;
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdu21 = (tempAtom->getX() + tempAtom->getY0() - Bx11) / 2;
	    }
	    else
	    {
		    dU0du21 = (tempAtom->getU1() - U0_11) / 2;
		    Bdu21 = (tempAtom->getX() + tempAtom->getY1() - Bx11) / 2;
	    }
    }
    ...
    else
    {
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdv21 = (Bx22 - tempAtom->getX() + tempAtom->getY0()) / 2;
	    }
	    else
	    {
		    dU0dv21 = (U0_22 - tempAtom->getU1()) / 2;
	    	    Bdv21 = (Bx22 - tempAtom->getX() + tempAtom->getY1()) / 2;
	    }
    }

    // Corner 12
    std::cout << "Corner 12" << std::endl;
    ...
    else
    {
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdu12 = (Bx22 - tempAtom->getX() + tempAtom->getY0()) / 2;
	    }
	    else
	    {
		    dU0du12 = (U0_22 - tempAtom->getU1()) / 2;
		    Bdu12 = (Bx22 - tempAtom->getX() + tempAtom->getY1()) / 2;
	    }
    }
    ...
    else
    {
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdv12 = (tempAtom->getX() + tempAtom->getY0() - Bx11) / 2;
	    }
	    else
	    {
		    dU0dv12 = (tempAtom->getU1() - U0_11) / 2;
		    Bdv12 = (tempAtom->getX() + tempAtom->getY1() - Bx11) / 2;
	    }
    }

    // Corner 22
    std::cout << "Corner 22" << std::endl;
	...
	std::cout << "else" << std::endl;
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdu22 = (tempAtom->getX() + tempAtom->getY0() - Bx12) / 2;
	    }
	    else
	    {
		    dU0du22 = (tempAtom->getU1() - U0_12) / 2;
		    Bdu22 = (tempAtom->getX() + tempAtom->getY1() - Bx12) / 2;
	    }
    }
	...
	std::cout << "else" << std::endl;
	    ...
	    if (side == 0)
	    {
		    ...
		    Bdv22 = (tempAtom->getX() + tempAtom->getY0() - Bx21) / 2;
	    }
	    else
	    {
		    dU0dv22 = (tempAtom->getU1() - U0_21) / 2;
		    Bdv22 = (tempAtom->getX() + tempAtom->getY1() - Bx21) / 2;
	    }
    }
    
    std::cout << "out of D block" << std::endl;*//*
    
    M3DQuadPrimitive atom01, atom02, atom10, atom20, atom31, atom32, atom13, atom23;
    
    //atom01
    if (ubase == 0)
	atom01 = quadAtom11;
    else
	atom01 = dynamic_cast<M3DQuadPrimitive*>(tempFigure->getPrimitivePtr(ubase-1, vbase);
    
    //atom02
    if (ubase == 0)
	atom02 = quadAtom12;
    else
	atom02 = dynamic_cast<M3DQuadPrimitive*>(tempFigure->getPrimitivePtr(ubase-1, vbase+1;
    
    //atom10
    if (vbase == 0)
	atom10 = quadAtom11;
    else
	atom10 = dynamic_cast<M3DQuadPrimitive*>(tempFigure->getPrimitivePtr(ubase, vbase-1);
    
    //atom20
    if (vbase == 0)
	atom20 = quadAtom21;
    else
	atom20 = dynamic_cast<M3DQuadPrimitive*>(tempFigure->getPrimitivePtr(ubase+1, vbase-1);
    
    //atom31
    if (ubase + 1 == tempFigure->getRowCount() - 1)
	atom31 = quadAtom21;
    else if (ubase == tempFigure ->getRowCount() - 1)
    ...
    Vector3D dBu11t = Bdu11 - (Bdu11 * Bn11) * Bn11;
    Vector3D dBu21t = Bdu21 - (Bdu21 * Bn21) * Bn21;
    Vector3D dBu12t = Bdu12 - (Bdu12 * Bn12) * Bn12;
    Vector3D dBu22t = Bdu22 - (Bdu22 * Bn22) * Bn22;
    
    Vector3D dBv11t = Bdv11 - (Bdv11 * Bn11) * Bn11;
    Vector3D dBv21t = Bdv21 - (Bdv21 * Bn21) * Bn21;
    Vector3D dBv12t = Bdv12 - (Bdv12 * Bn12) * Bn12;
    Vector3D dBv22t = Bdv22 - (Bdv22 * Bn22) * Bn22;
    
    double Bhx[16] = { Bx11.getX(), Bx21.getX(), dBu11t.getX(), dBu21t.getX(), 
		Bx12.getX(), Bx22.getX(), dBu12t.getX(), dBu22t.getX(), dBv11t.getX(), 
		dBv21t.getX(), 0, 0, dBv21t.getX(), dBv22t.getX(), 0, 0};

    double Bhy[16] = { Bx11.getY(), Bx21.getY(), dBu11t.getY(), dBu21t.getY(), 
		Bx12.getY(), Bx22.getY(), dBu12t.getY(), dBu22t.getY(), dBv11t.getY(), 
		dBv21t.getY(), 0, 0, dBv21t.getY(), dBv22t.getY(), 0, 0};

    double Bhz[16] = { Bx11.getZ(), Bx21.getZ(), dBu11t.getZ(), dBu21t.getZ(), 
		Bx12.getZ(), Bx22.getZ(),dBu12t.getZ(), dBu22t.getZ(), dBv11t.getZ(), 
		dBv21t.getZ(), 0, 0, dBv21t.getZ(), dBv22t.getZ(), 0, 0};
		
		
    Matrix Bhxmat = Matrix(4, 4, Bhx, true);
    Matrix Bhymat = Matrix(4, 4, Bhy, true);
    Matrix Bhzmat = Matrix(4, 4, Bhz, true);
    
    double Bhu[4] = { h1(u), h2(u), h3(u), h4(u) };
    double Bhv[4] = { h1(v), h2(v), h3(v), h4(v) };
    Matrix Bhumat = Matrix(1, 4, Bhu, true);
    Matrix Bhvmat = Matrix(4, 1, Bhv, true);
    
    Matrix xn = Bhumat * Bhxmat * Bhvmat;
    Matrix yn = Bhumat * Bhymat * Bhvmat;
    Matrix zn = Bhumat * Bhzmat * Bhvmat;
    
    Vector3D newB = Vector3D(xn(0,0), yn(0,0), zn(0,0));
   ...
   // Get unit normals for the four corners and project derivatives on to the tangent plane
	...
	//Do Hermite interpolation for boundary points -- used for testing/debugging purposes
	Vector3D Bn11 = U0_11;
	Vector3D Bn21 = U0_21;
	Vector3D Bn12 = U0_12;
	Vector3D Bn22 = U0_22;*/
