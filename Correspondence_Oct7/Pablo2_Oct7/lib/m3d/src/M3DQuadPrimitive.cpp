...
M3DQuadPrimitive(double x0, double x1, double x2,
                 ...
                 double UEndx, double UEndy, double UEndz, double deltaU0, double deltaU1, double deltaU2, double deltaV0, double deltaV1, double deltaV2)
                 : M3DPrimitive(x0, x1, x2), r0(R0), r1(R1),
                   u0(U0x, U0y, U0z), u1(U1x, U1y, U1z), uEnd(UEndx, UEndy, UEndz),deltaU0(deltaU0),deltaU1(deltaU1),deltaU2(deltaU2),deltaV0(deltaV0),deltaV1(deltaV1),deltaV2(deltaV2)  //Liyun add deltaU,deltaV
{ 
    //check_r0();
}
...
//--------Liyun--------------------
//void M3DQuadPrimitive::setDeltaU( float _deltaU )
//{
  //  deltaU = _deltaU;
//}
//void M3DQuadPrimitive::setDeltaV( float _deltaV )
//{
  //  deltaV = _deltaV;
//}

//add by liyun Oct 23, 2013
void M3DQuadPrimitive::setR0( double _r0 ){
    r0 = _r0;
}

//add by liyun Oct 23, 2013
void M3DQuadPrimitive::setR1( double _r1 ){
    r1 = _r1;
}


//add by liyun Nov 4, 2013
void M3DQuadPrimitive::setDeltaU0( double _du0 ){
    deltaU0 = _du0;
}
void M3DQuadPrimitive::setDeltaU1( double _du1 ){
    deltaU1 = _du1;
}
void M3DQuadPrimitive::setDeltaU2( double _du2 ){
    deltaU2 = _du2;
}
void M3DQuadPrimitive::setDeltaV0( double _dv0 ){
    deltaV0 = _dv0;
}
void M3DQuadPrimitive::setDeltaV1( double _dv1 ){
    deltaV1 = _dv1;
}
void M3DQuadPrimitive::setDeltaV2( double _dv2 ){
    deltaV2 = _dv2;
}
...
M3DQuadPrimitive* readSrepPrimitive(Registry& registry, const char * newStr)
{
    ...
    //---------------------Liyun------------
   // double deltaU = registry.getFloatValue(newStr, 0.0, "deltaU");
  //  double deltaV = registry.getFloatValue(newStr, 0.0, "deltaV");

    double deltaU0 = registry.getDoubleValue(newStr, 0.0, "deltaU[0]");
    double deltaU1 = registry.getDoubleValue(newStr, 0.0, "deltaU[1]");
    double deltaU2 = registry.getDoubleValue(newStr, 0.0, "deltaU[2]");
    double deltaV0 = registry.getDoubleValue(newStr, 0.0, "deltaV[0]");
    double deltaV1 = registry.getDoubleValue(newStr, 0.0, "deltaV[1]");
    double deltaV2 = registry.getDoubleValue(newStr, 0.0, "deltaV[2]");
       ...
       strcmp(typeStr, M3D_END_PRIMITIVE_STR) == 0) 
    {
        primPtr = new M3DQuadEndPrimitive(
            ...
            uEndx, uEndy, uEndz,deltaU0, deltaU1, deltaU2, deltaV0, deltaV1, deltaV2);//Liyun, add deltaU, deltaV
    } else {
...
void M3DQuadPrimitive::writePrimitive(Registry& registry, const char * regStr, ...) const
{
    ...
    //registry.setDoubleValue(newStr, getREnd(), "r[2]");
    ...
    registry.setDoubleValue(newStr, getDeltaU0(), "deltaU[0]");
    registry.setDoubleValue(newStr, getDeltaU1(), "deltaU[1]");
    registry.setDoubleValue(newStr, getDeltaU2(), "deltaU[2]");
    registry.setDoubleValue(newStr, getDeltaV0(), "deltaV[0]");
    registry.setDoubleValue(newStr, getDeltaV1(), "deltaV[1]");
    registry.setDoubleValue(newStr, getDeltaV2(), "deltaV[2]");

    registry.setIntValue(newStr, getSelectionFlags(), "selected");

    //liyun
    //cout<<"--------------r[0]---------------"<<registry.getDoubleValue(newStr, 1.0, "r[0]")<<endl;
   // cout<<"--------------r[1]---------------"<<registry.getDoubleValue(newStr, 1.0, "r[1]")<<endl;
