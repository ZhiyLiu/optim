% =====================================================================
% Function Param = readParam(filePath)
%
%   Read filePath and get the setting to run 'getResidueStat.m'
%   filePath : specifies a 'param.txt' file.
% ---------------------------------------------------------------------
function Param = readParam(filePath)
...
while (~isThere)
    beep;
    uiwait(errordlg(['Missing parameter file: ' filePath]));
    
    prompt = 'Enter filename of ''param.txt'' file:';
    answer = inputdlg(prompt, 'Select file', 1, {filePath});
    if (isempty(answer))
        beep;
...
m = textread(filePath,'%s','delimiter','\n','whitespace','', 'commentstyle','matlab', 'bufsize', 2*4095);
...
worldExtent = 0;
setVal = sscanf(strParam(strmatch('worldExtent', strParam), :), 'worldExtent = %f');
if (~isempty(setVal))
    worldExtent = setVal;
end
weightMode = 1;  
setVal = sscanf(strParam(strmatch('weightMode', strParam), :), 'weightMode = %d');
if (~isempty(setVal))
    weightMode = setVal;
end
...
Param = struct('statsFlag', statsFlag, 'bReadData', bReadData, 'PGAflag', PGAflag, 'bOutput', bOutput, ...
	...
	'normalizeEndAtoms', normalizeEndAtoms, 'worldExtent', worldExtent, 'weightMode', weightMode);
